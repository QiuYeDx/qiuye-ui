...
- 稳定字符槽控制 width；槽内字形按单调递增的出现序号切换，各自退出完成后独立清理。变宽时不缩放文字。
...
## 高频循环入场方向修复（2026-09-24）

- 复现：每 40ms 递增，持续 120 次。旧实现初轮个位 5 从下方进入；到 25 时新 5 已在基线上方约 15.7px，个位 DOM 保留十个字符副本。
- 原因：字符值 key 会复用尚留在 AnimatePresence 退出集合中的上一轮节点；持续有新退出动画时，透明的旧节点仍未从集合移除。重复字符重新出现便沿旧退场位置返回，不再执行新入场。
- 修复：数位 key 不变，字形使用槽内单调递增的出现序号。用 NumberGlyphs 管理短暂重叠的字形，每个副本在自己的退出动画完成时移除；冻结其退场方向，不受后续反向更新重定向。
- 保留独立的 initial 边界，避免祖先数位 AnimatePresence 的 initial=false 让后续新字形跳过入场。初始字形自身 initial=false，新增字形仍从正确一侧透明进入。
- 保留宽度/字面分层、稳定 transform、blur 默认开启与开关能力。只在该位字符变化时更新出现序号，未变化数位不重建。
- 新增可手动开始、停止的连续更新 Demo：每 40ms 更新一次、120 次后自动结束，支持递增/递减，复用模糊与时长控制；卸载清理定时器。
- 浏览器对比：关闭模糊、280ms 过渡，完整 120 次递增采样没有当前字形进入基线上方；1000ms + blur 开启的 120 次递减也没有方向反转，个位副本峰值 27，停止后回到 1。默认时长副本峰值 9，未随轮数增长；最终每位 opacity=1、y=0。
- 快速 1000→9→10 后只有两个数位、各一个字形；不能只测试一轮或只检查最终静态截图。
